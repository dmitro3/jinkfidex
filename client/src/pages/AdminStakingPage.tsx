...
import { CheckCircle2, XCircle, Clock, ExternalLink, ChevronDown, ChevronUp, Coins } from "lucide-react";
...
function InfoRow({ label, value }: { label: string; value: string | number }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.3rem 0", borderBottom: "1px solid var(--border)" }}>
      ...
      <span style={{ fontSize: 11, color: "var(--text)", fontFamily: "'Share Tech Mono', monospace" }}>{value}</span>
    </div>
  );
}
...
export default function AdminStakingPage() {
  ...
  const { data: submissions = [], isLoading } = useQuery({
    queryKey: ["admin-staking-submissions", filter],
    queryFn: () => stakingApi.listSubmissions(filter === "ALL" ? undefined : filter),
    retry: false,
  });

  const approveMut = useMutation({
    mutationFn: ({ id, note }: { id: string; note?: string }) => stakingApi.approveSubmission(id, note),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin-staking-submissions"] }),
  });

  const rejectMut = useMutation({
    mutationFn: ({ id, note }: { id: string; note?: string }) => stakingApi.rejectSubmission(id, note),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["admin-staking-submissions"] }),
  });
  ...
  (submissions as StakingSubmission[]).forEach(s => { if (s.status in counts) counts[s.status as keyof typeof counts]++; });
    ...
    <div style={{ maxWidth: 1000, margin: "0 auto", padding: "2rem 1.5rem" }}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: 11, margin: 0, fontFamily: "'Share Tech Mono', monospace" }}>
          Admin panel — review, approve, or reject third-party staking pool submissions
        </p>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        {(submissions as StakingSubmission[]).map(sub => {
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 800, fontSize: 14, fontFamily: "'Rajdhani', sans-serif", marginBottom: "0.15rem" }}>
                    {sub.tokenSymbol} / {sub.rewardTokenSymbol}
                  ...
                  <div style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "'Share Tech Mono', monospace", display: "flex", gap: "1rem", flexWrap: "wrap" }}>
                    ...
                    <span>{sub.apy}% APY</span>
                    <span>{sub.lockDays}d lock</span>
                ...
                <div style={{ borderTop: "1px solid var(--border)", padding: "1.25rem" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1.5rem", marginBottom: "1.25rem" }}>
                    ...
                    <div>
                      ...
                      <InfoRow label="NAME" value={sub.tokenName} />
                      <InfoRow label="CHAIN" value={CHAIN_NAMES[sub.chainId] ?? `Chain ${sub.chainId}`} />
                    </div>

                    {/* Reward info */}
                    <div>
                      <div style={{ fontSize: 9, letterSpacing: "0.18em", color: "var(--text-muted)", fontFamily: "'Share Tech Mono', monospace", marginBottom: "0.5rem" }}>REWARD</div>
                      <InfoRow label="REWARD TOKEN" value={sub.rewardTokenSymbol} />
                      <InfoRow label="REWARD ADDR" value={trunc(sub.rewardTokenAddress)} />
                      <InfoRow label="APY" value={`${sub.apy}%`} />
                      <InfoRow label="TOTAL BUDGET" value={sub.totalRewardBudget} />
                    </div>

                    {/* Pool parameters */}
                    <div>
                      <div style={{ fontSize: 9, letterSpacing: "0.18em", color: "var(--text-muted)", fontFamily: "'Share Tech Mono', monospace", marginBottom: "0.5rem" }}>PARAMETERS</div>
                      <InfoRow label="LOCK PERIOD" value={`${sub.lockDays} days`} />
                      <InfoRow label="MIN STAKE" value={sub.minStake} />
                      <InfoRow label="MAX STAKE" value={sub.maxStake ?? "Unlimited"} />
                      <InfoRow label="STARTS" value={fmtDate(sub.poolStartDate)} />
                      <InfoRow label="ENDS" value={fmtDate(sub.poolEndDate)} />
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: "1.25rem" }}>
                    ...
                    <p style={{ fontSize: 12, color: "var(--text)", lineHeight: 1.7, margin: 0, padding: "0.6rem 0.75rem", background: "var(--bg-input)" }}>{sub.description}</p>
                  </div>
